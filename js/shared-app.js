...
function setupDashboardNav(downloadPrefix) {
	...
	document.getElementById('download-donut').onclick = () => {
		...
		const slug = range.title
			.replace(/[^a-zA-Z0-9]+/g, '-')
			.replace(/^-+|-+$/g, '');
		exportDonutAsPng(`${downloadPrefix}-${slug}.png`);
	};
}

// Rasterizes the dashboard's live <svg> (built by drawDonutChartCore) onto
// an offscreen canvas at 2x its own size for a crisp save, then hands the
// PNG to the Web Share API so it saves straight to Photos on iOS. Falls
// back to a plain download link on desktop / browsers without file sharing.
// `link.href = canvas.toDataURL(...)` used to do this directly, but a
// data: URL "navigation" is exactly the kind of thing that can get caught
// by a Home Screen web app's scope handling - this never navigates at all.
async function exportDonutAsPng(filename) {
	const wrap = document.getElementById('donut-canvas');
	...
	try {
		...
		// The SVG itself is transparent (the black card background comes from
		// CSS), so paint it in manually or the exported PNG would have a
